...
Output format: Just write the explanation directly. No JSON, no markdown, no formatting.`;
        ...
        const responseText = await this.chat(systemPrompt, `Query: "${input.query}"\n\nResult:\n${resultStr}`, { temperature: 0.7, maxTokens: 300 });
...
Return ONLY the JSON object, no markdown or extra text.`;
        const resultStr = JSON.stringify(input.executionResult, null, 2).substring(0, 2000);
        const responseText = await this.chat(systemPrompt, `Original Query: "${input.query}"\n\nExecution Result:\n${resultStr}\n\nGenerated Explanation: ${input.explanation}`, { temperature: 0.3, maxTokens: 500 });
        try {

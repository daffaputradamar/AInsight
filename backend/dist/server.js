...
import { createAdapterForSession, hasSessionConfig, configureSession, getSessionConfig } from './adapters/postgres.js';
...
async function handleQuery(req, res) {
    ...
    try {
        ...
        const modelOverride = req.headers['x-ai-model'];
...
async function handleGetDbConfig(req, res) {
    try {
        const sessionId = getSessionId(req);
...
async function handleSetDbConfig(req, res) {
    let body = '';
    await new Promise((resolve) => {
        req.on('data', (chunk) => {
            body += chunk;

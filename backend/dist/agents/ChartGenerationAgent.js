...
Respond with JSON only:
{
  ...
  "reasoning": "Brief explanation of why this chart type and these axes were chosen"
        ...
        try {
            const result = await this.chatJSON(systemPrompt, `User query: "${input.query}"\n\nData explanation: ${input.explanation}\n\nSample data (first 5 rows):\n${sampleData}`, ChartSpecOutputSchema, { temperature: 0.3, maxTokens: 400 });
            return {
                visualizationSpec: {
                    type: result.type,
                    title: result.title,
    ...
    analyzeColumns(data, columns) {
        ...
        for (const col of columns) {
            ...
            if (isDate)
                ...
                type = `categorical (${uniqueCount} unique values)`;
     ...
     */
    fallbackChartSpec(query, columns, data) {
...
//# sourceMappingURL=ChartGenerationAgent.js.map
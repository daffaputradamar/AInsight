...
async function main() {
    ...
    try {
        ...
        const insights = insightsResponse.output;
        ...
        const askQuestion = () => {
            rl.question('Query: ', async (query) => {
                ...
                try {
                    ...
                    // Stage 1
                    const understanding = result.responses[0]?.output;
                    ...
                    // Stage 2
                    const codeGen = result.responses[1]?.output;
                    console.log('2. Code Generation:');
                    console.log(`   Language: ${codeGen?.language}`);
                    console.log(`   Code:\n${codeGen?.code}\n`);
                    // Stage 3
                    const execution = result.responses[2]?.output;
                    ...
                    // Stage 4
                    const reasoning = result.responses[3]?.output;
                    ...
                    if (result.finalResult) {
                        ...
                        console.log(JSON.stringify({
                            dataRows: result.finalResult.data?.length || 0,
                            explanation: result.finalResult.explanation,
                            executionTime: result.finalResult.executionTime,
                        }, null, 2));
                    }
                    console.log('\n---\n');
                }

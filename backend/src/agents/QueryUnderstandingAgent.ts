...
export class QueryUnderstandingAgent extends Agent {
  ...
  private async classify(input: ClassifyInput): Promise<QueryUnderstandingOutput> {
...
- Keep chatResponse concise and helpful`;
  ...
  private getDefaultChatResponse(query: string): string {
  ...
  private inferIntent(query: string): string {
    if (query.includes('count') || query.includes('how many')) return 'counting';
    if (query.includes('sum') || query.includes('total')) return 'aggregation';
    if (query.includes('average') || query.includes('mean')) return 'statistical';
    if (query.includes('trend') || query.includes('over time')) return 'temporal';
